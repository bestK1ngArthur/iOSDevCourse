...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSMutableArray *specialSentences = [NSMutableArray array];
    
    for (NSString *sentence in sentences) {
        NSRange range = [sentence rangeOfString:@"NSString"];
        if (range.location != NSNotFound) {
            [specialSentences addObject:sentence];
        }
    }
    
    //Master
    
    NSMutableArray *newSpecialSentences = [NSMutableArray array];
    
    for (NSString *sentence in specialSentences) {
        
        NSArray *words = [sentence componentsSeparatedByString:@" "];
        NSEnumerator *enumerator = [words reverseObjectEnumerator];
        
        NSMutableArray *newWords = [NSMutableArray array];
        
        for (id element in enumerator) {
            [newWords addObject:element];
        }
        
        NSString *newSentence = [newWords componentsJoinedByString:@" "];
        [newSpecialSentences addObject:newSentence];
        
    }
    
    NSLog(@"%@", newSpecialSentences);
    */
    ...
    for (NSString *word in words) {
        ...
        NSInteger lastCharIndex = [word length] - 1;
        NSString *newWord = word;
        
        if (lastCharIndex > 0) {
            NSString *wordWithoutLastChar = [word substringToIndex:lastCharIndex];
            NSString *lastChar = [[word substringFromIndex:lastCharIndex] uppercaseString];
            newWord = [wordWithoutLastChar stringByAppendingString:lastChar];
